...
if [ "$#" -ne 1 ] || [ $1 -lt 0 ] || [ $1 -gt 5 ]; then
    echo "Usage: ./run-morpheus-variant.sh <variant-id>"
    echo "Variant: 1=spec2, 2=spec2-no-pe, 3=spec1, 4=spec1-no-pe, 5=no deduction"
    echo "e.g. ./run-morpheus-variant.sh 1"
    exit 1
fi
